...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on timetables documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in timetables.find({}):
        scanned += 1
        current = normalize_schema_version(document.get("schema_version"))
        if current == TIMETABLE_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[timetables] {document['_id']}: "
            f"class_id={document.get('class_id')} "
            f"semester={document.get('semester')} "
            f"version={document.get('version')} "
            f"schema_version={document.get('schema_version')} -> {TIMETABLE_SCHEMA_VERSION}"
        )

        if args.apply:
            result = timetables.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": TIMETABLE_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={TIMETABLE_SCHEMA_VERSION}"
    )
    return 0
