...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on departments documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in departments.find({}):
        scanned += 1
        current = normalize_schema_version(document.get("schema_version"))
        if current == DEPARTMENT_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[departments] {document['_id']}: "
            f"code={document.get('code')} "
            f"name={document.get('name')} "
            f"schema_version={document.get('schema_version')} -> {DEPARTMENT_SCHEMA_VERSION}"
        )

        if args.apply:
            result = departments.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": DEPARTMENT_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={DEPARTMENT_SCHEMA_VERSION}"
    )
    return 0
