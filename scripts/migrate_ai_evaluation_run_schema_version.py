...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on ai_evaluation_runs documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    runs = database["ai_evaluation_runs"]
    ...
    for document in runs.find({}):
        ...
        if current == AI_EVALUATION_RUN_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[ai_evaluation_runs] {document['_id']}: "
            f"schema_version={document.get('schema_version')} -> {AI_EVALUATION_RUN_SCHEMA_VERSION}"
        )

        if args.apply:
            result = runs.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": AI_EVALUATION_RUN_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={AI_EVALUATION_RUN_SCHEMA_VERSION}"
    )
    return 0
