...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on assignments documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in assignments.find({}):
        scanned += 1
        current = normalize_schema_version(document.get("schema_version"))
        if current == ASSIGNMENT_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[assignments] {document['_id']}: "
            f"title={document.get('title')} "
            f"schema_version={document.get('schema_version')} -> {ASSIGNMENT_SCHEMA_VERSION}"
        )

        if args.apply:
            result = assignments.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": ASSIGNMENT_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={ASSIGNMENT_SCHEMA_VERSION}"
    )
    return 0
