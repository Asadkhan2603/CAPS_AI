...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on notices documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in notices.find({}):
        scanned += 1
        current = normalize_schema_version(document.get("schema_version"))
        if current == NOTICE_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[notices] {document['_id']}: "
            f"title={document.get('title')} "
            f"schema_version={document.get('schema_version')} -> {NOTICE_SCHEMA_VERSION}"
        )

        if args.apply:
            result = notices.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": NOTICE_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={NOTICE_SCHEMA_VERSION}"
    )
    return 0
