...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on students documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in students.find({}):
        ...
        if current == STUDENT_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[students] {document['_id']}: "
            f"roll_number={document.get('roll_number')} "
            f"full_name={document.get('full_name')} "
            f"schema_version={document.get('schema_version')} -> {STUDENT_SCHEMA_VERSION}"
        )

        if args.apply:
            result = students.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": STUDENT_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={STUDENT_SCHEMA_VERSION}"
    )
    return 0
