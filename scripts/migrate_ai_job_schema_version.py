...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on ai_jobs documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in jobs.find({}):
        ...
        if current == AI_JOB_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[ai_jobs] {document['_id']}: "
            f"schema_version={document.get('schema_version')} -> {AI_JOB_SCHEMA_VERSION}"
        )

        if args.apply:
            result = jobs.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": AI_JOB_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={AI_JOB_SCHEMA_VERSION}"
    )
    return 0
