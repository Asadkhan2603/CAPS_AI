...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on enrollments documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in enrollments.find({}):
        scanned += 1
        current = normalize_schema_version(document.get("schema_version"))
        if current == ENROLLMENT_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[enrollments] {document['_id']}: "
            f"class_id={document.get('class_id')} "
            f"student_id={document.get('student_id')} "
            f"schema_version={document.get('schema_version')} -> {ENROLLMENT_SCHEMA_VERSION}"
        )

        if args.apply:
            result = enrollments.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": ENROLLMENT_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={ENROLLMENT_SCHEMA_VERSION}"
    )
    return 0
