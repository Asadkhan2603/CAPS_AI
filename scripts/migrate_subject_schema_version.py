...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on subjects documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in subjects.find({}):
        scanned += 1
        current = normalize_schema_version(document.get("schema_version"))
        if current == SUBJECT_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[subjects] {document['_id']}: "
            f"code={document.get('code')} "
            f"name={document.get('name')} "
            f"schema_version={document.get('schema_version')} -> {SUBJECT_SCHEMA_VERSION}"
        )

        if args.apply:
            result = subjects.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": SUBJECT_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={SUBJECT_SCHEMA_VERSION}"
    )
    return 0
