...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on clubs documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in clubs.find({}):
        scanned += 1
        current = normalize_schema_version(document.get("schema_version"))
        if current == CLUB_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[clubs] {document['_id']}: "
            f"name={document.get('name')} "
            f"schema_version={document.get('schema_version')} -> {CLUB_SCHEMA_VERSION}"
        )

        if args.apply:
            result = clubs.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": CLUB_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={CLUB_SCHEMA_VERSION}"
    )
    return 0
