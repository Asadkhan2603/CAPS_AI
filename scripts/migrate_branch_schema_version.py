...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on legacy branches documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in branches.find({}):
        scanned += 1
        current = normalize_schema_version(document.get("schema_version"))
        if current == BRANCH_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[branches] {document['_id']}: "
            f"department_code={document.get('department_code')} "
            f"name={document.get('department_name') or document.get('name')} "
            f"schema_version={document.get('schema_version')} -> {BRANCH_SCHEMA_VERSION}"
        )

        if args.apply:
            result = branches.update_one(
                {"_id": document["_id"]},
    ...
    print(
        ...
        f"target_version={BRANCH_SCHEMA_VERSION}"
    )
    return 0
