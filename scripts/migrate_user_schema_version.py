...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on users documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in users.find({}):
        ...
        candidates += 1
        print(
            f"[users] {document['_id']}: "
            f"email={document.get('email')} "
            f"role={document.get('role')} "
            f"schema_version={document.get('schema_version')} -> {USER_SCHEMA_VERSION}"
        )

        if args.apply:
            result = users.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": USER_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={USER_SCHEMA_VERSION}"
    )
    return 0
