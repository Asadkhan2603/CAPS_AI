...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on submissions documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in submissions.find({}):
        scanned += 1
        current = normalize_schema_version(document.get("schema_version"))
        if current == SUBMISSION_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[submissions] {document['_id']}: "
            f"schema_version={document.get('schema_version')} -> {SUBMISSION_SCHEMA_VERSION}"
        )

        if args.apply:
            result = submissions.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": SUBMISSION_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={SUBMISSION_SCHEMA_VERSION}"
    )
    return 0
