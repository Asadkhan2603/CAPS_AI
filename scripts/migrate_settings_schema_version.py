...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on settings documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in settings_collection.find({}):
        scanned += 1
        ...
        candidates += 1
        print(
            f"[settings] {document['_id']}: "
            f"key={document.get('key')} "
            f"schema_version={document.get('schema_version')} -> {SETTINGS_SCHEMA_VERSION}"
        )

        if args.apply:
            result = settings_collection.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": SETTINGS_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={SETTINGS_SCHEMA_VERSION}"
    )
    return 0
