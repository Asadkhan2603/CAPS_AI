...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill schema_version on classes documents."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    for document in classes.find({}):
        ...
        if current == CLASS_SCHEMA_VERSION:
            continue

        candidates += 1
        print(
            f"[classes] {document['_id']}: "
            f"name={document.get('name')} "
            f"batch_id={document.get('batch_id')} "
            f"semester_id={document.get('semester_id')} "
            f"schema_version={document.get('schema_version')} -> {CLASS_SCHEMA_VERSION}"
        )

        if args.apply:
            result = classes.update_one(
                {"_id": document["_id"]},
                {"$set": {"schema_version": CLASS_SCHEMA_VERSION}},
            )
            modified += int(result.modified_count)

    print(
        f"Summary: scanned={scanned} candidates={candidates} modified={modified} "
        f"target_version={CLASS_SCHEMA_VERSION}"
    )
    return 0
